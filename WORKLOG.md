...
- **การแก้ปัญหาชนิดข้อมูลฝั่งฐานข้อมูลจริง (MySQL String Driver Coercion Fix)**:
...
- **ระบบให้คะแนนแบบกลุ่ม (Bulk Grading System)**:
  - แก้ไขความซับซ้อนและอำนวยความสะดวกในการให้คะแนนเด็กนักเรียนจำนวนมาก (เช่น ให้คะแนนเต็ม 10 ทั้งห้อง หรือเช็คสมุดส่งครบทุกคนพร้อมกัน)
  - พัฒนาระบบปุ่มลัด **"☑️ เลือกทั้งหมด (Select All)"** และ **"⬜ ล้าง"** บนแถบควบคุมการเลือกนักเรียน เพื่อติ๊กเลือกเด็กทั้งห้องเรียนได้ในคลิกเดียว
  - เพิ่มหน้าต่างป๊อปอัปให้คะแนนกลุ่ม **"✍️ ให้คะแนนกลุ่ม (Bulk Grade Modal)"** เพื่อให้ครูสามารถเลือกใบงานชิ้นที่ต้องการให้คะแนน พิมพ์คะแนน หรือใช้ปุ่มลัดด่วนป้อนคะแนนเต็ม (สืบค้นจากคะแนนสูงสุดของงานชิ้นนั้นในห้องโดยอัตโนมัติ), ติ๊กเช็คส่งสมุดด่วน (`-1`), หรือเคลียร์คะแนนทั้งหมดเป็น `0`
  - ทำการส่งคำสั่งเซฟแบบกลุ่มด้วย Prisma transaction รวดเร็วปลอดภัยเสถียร 100%

- **การแก้ไขบั๊กโหมดตรวจสมุดด่วนแสดงผลหน้าเขียวทันที (Quick-Check validation fix)**:
  - แก้ไขปัญหาเมื่อเปิดใช้งานโหมดตรวจสมุดด่วนครั้งแรก (หรือหลังจากล้าง `localStorage` แล้วไม่มีใบงานที่ถูกเลือก) ระบบจะแสดงผลหน้าจอสีเขียว "เช็คชื่อส่งสมุดสำเร็จ!" ขึ้นมาล็อกหน้าจอทันทีโดยไม่ได้เซฟคะแนนจริง
  - *สาเหตุ*: เมื่อไม่มีใบงานที่บันทึกอยู่ใน `localStorage` ค่า `activeTaskId` จะถูกดึงออกมาเป็น `null` หรือสายอักขระว่างเปล่า ส่งผลให้ตัวจับการอัปโหลดข้อมูล (Auto-save Effect) ข้ามเงื่อนไขความปลอดภัยและวิ่งไปทริกเกอร์ `setAutoSaved(true)`
  - *แนวทางแก้ไข*: ทำการตรวจสอบใบงานที่ดึงมาจาก `localStorage` เสมอ หากไม่มีใบงานนั้นอยู่ในรายชื่อใบงานจริงของห้องเรียน (`tasks`) ระบบจะป้อนค่าใบงานแรกสุดของห้องเรียนเข้าไปเป็นค่าเริ่มต้น (`tasks[0].id`) แทน ช่วยสยบปัญหาหน้าเขียวล็อกหน้าจอแบบผิดจังหวะได้ 100%

- **การแก้ปัญหาคะแนนเต็มแสดงผลเป็นติดลบ (เต็ม -1)**:
  - แก้ไขปัญหาหน้าจอตรวจคะแนนด่วนของครูแสดงคะแนนเต็มในบางใบงานผิดพลาดเป็น `เต็ม -1`
  - *สาเหตุ*: ระบบจะคำนวณคะแนนเต็มของใบงานแต่ละชิ้นโดยอ้างอิงจากคะแนนสูงสุดที่เคยเก็บในตาราง `Score` ของห้องเรียนนั้น แต่เมื่อมีการนำระบบตรวจสมุดด่วนเข้ามาใช้งาน และใบงานชิ้นนั้นยังไม่มีใครได้รับคะแนนอื่นนอกจากค่าตรวจสมุดพิเศษ `-1` ทำให้ค่าที่สูงที่สุดที่สืบค้นเจอคือ `-1` ระบบจึงแสดงผลเป็นคะแนนเต็มติดลบ
  - *แนวทางแก้ไข*: แก้ไขไฟล์ [page.tsx](file:///Users/kanokkarn/Data/AI%20Title/projects/homework-next/src/app/grade/%5BroomId%5D/%5BstudentId%5D/page.tsx) โดยเพิ่มเงื่อนไข `value: { gt: -1 }` เข้าไปในขั้นตอนค้นหาข้อมูลผ่านคำสั่ง `prisma.score.groupBy` เพื่อละเว้นค่าคะแนนพิเศษ `-1` จากการนำมาคำนวณหาคะแนนเต็มของใบงาน ทำให้ใบงานที่ยังไม่มีคะแนนจริงกลับมาแสดงคะแนนเต็มเริ่มต้นที่ 10 ได้อย่างถูกต้องและเหมาะสม

- **ระบบกำหนดคะแนนเต็มของใบงานโดยตรง (Explicit Task Max Score)**:
  - พัฒนาระบบให้สามารถตั้งค่าและบันทึกคะแนนเต็มของใบงานแต่ละชิ้นลงในฐานข้อมูลได้โดยตรง แทนการประมาณค่าสูงสุดแบบไดนามิกแบบเดิม
  - *การเปลี่ยนแปลงด้าน Database*: เพิ่มคอลัมน์ `maxScore Float @default(0)` ในตาราง `Task` ของ Prisma Schema (ปรับเปลี่ยนจาก 10 เป็น 0 ตามความต้องการของคุณครู)
  - *การเปลี่ยนแปลงด้าน UI/UX*:
    - ในแถบเมนูจัดการใบงานห้องเรียน เพิ่มช่องให้กรอกคะแนนเต็มควบคู่กับการป้อนชื่อตอน "เพิ่มใบงานใหม่" (ปรับปรุงค่าเริ่มต้นแสดงเป็น `0` คะแนน และอนุญาตให้ป้อนตั้งแต่ `0` คะแนนขึ้นไปได้)
    - ในรายการใบงาน เพิ่มช่องสำหรับป้อนหรือแก้ไขคะแนนเต็มสำหรับแต่ละใบงาน (แสดงค่าเริ่มต้นเป็น `0` และแก้ไขค่าต่ำสุดเป็น `0` เช่นกัน)
    - ปรับปรุงแถบเมนูให้คะแนนกลุ่มแบบ Bulk ให้ดึงค่าคะแนนเต็มของใบงานนั้นๆ มากรอกให้อัตโนมัติเมื่อครูกดปุ่ม "💯 คะแนนเต็ม" โดยไม่ต้องเปรียบเทียบจากคะแนนของนักเรียนในห้องอีกต่อไป
    - ปรับปรุงให้หน้าตรวจคะแนนด่วนดึงค่า `maxScore` ของงานมาแสดงผลตรงๆ ในรูปแบบ `เต็ม X`

- **การป้องกันการตรวจสมุดด่วนทำงานทันทีเมื่อกดเปิดโหมด (Manual Toggle Auto-Save Prevention)**:
  - แก้ไขปัญหาเมื่อครูทำการเปิดสวิตช์ใช้งาน "โหมดสแกนตรวจสมุดด่วน" บนหน้าจอของนักเรียนคนใดคนหนึ่งเป็นครั้งแรก หน้าจอความสำเร็จสีเขียวจะเด้งปิดทับทันทีเพื่อบันทึกส่งงานอัตโนมัติของคนนั้นโดยที่ครูยังไม่มีโอกาสเลือกชิ้นงานตรวจด่วนเลย
  - *สาเหตุ*: เมื่อเปิดสวิตช์ `quickCheckActive` ให้เป็น `true` ระบบจับการเปลี่ยนแปลงอัตโนมัติ (Auto-save Effect) จะประมวลผลทันที และเลือกเซฟใบงานแรกให้เด็กทันทีเนื่องจากสถานะ `hasAutoChecked` ยังเป็นค่าว่าง
  - *แนวทางแก้ไข*: แก้ไขโค้ดใน [QuickGradeClient.tsx](file:///Users/kanokkarn/Data/AI%20Title/projects/homework-next/src/app/grade/%5BroomId%5D/%5BstudentId%5D/QuickGradeClient.tsx) โดยทำการอัปเดตสถานะ `hasAutoChecked` ให้มีค่าเป็นไอดีของนักเรียนปัจจุบันทันทีเมื่อมีการกดสวิตช์เปิดโหมดด้วยตนเอง เพื่อป้องกันไม่ให้ระบบวิ่งตรวจสมุดด่วนให้คนปัจจุบันทันที (จะเก็บไว้ทำงานเฉพาะเมื่อโหลดหน้านี้ขึ้นมาใหม่จาก QR Code ของนักเรียนคนถัดไปเท่านั้น)

- **ย้ายแผงตั้งค่าโหมดตรวจสมุดด่วนไปที่ Dashboard ห้องเรียนหลัก (Relocate Quick Check-in Settings to Room Dashboard)**:
  - แก้ไขปัญหาความสับสนในการเปิดปิดโหมดและเปลี่ยนใบงานตรวจด่วน โดยทำการย้ายระบบควบคุมทั้งหมดออกจากหน้าคะแนนรายบุคคล ไปรวมไว้ที่หน้าแดชบอร์ดหลักของห้องเรียนเพื่อให้เป็นระบบและปลอดภัยยิ่งขึ้น
  - *การเปลี่ยนแปลงด้าน UI หน้าห้องหลัก*:
    - ย้ายปุ่มสวิตช์เปิด/ปิดโหมดสแกนตรวจด่วน และดรอปดาวน์เลือกชิ้นงาน ไปติดตั้งไว้ที่ส่วนล่างสุดของแถบเครื่องมือข้าง **"จัดการห้องเรียน (Action Sheet Sidebar)"**
    - เพิ่มแบนเนอร์แสดงสถานะพร้อมไฟสัญญาณกะพริบสีเขียว (Active Pulse Badge) ถัดจากปุ่มควบคุมหลักของห้องเรียน เพื่อให้ทราบได้ทันทีว่าโหมดตรวจด่วนกำลังทำงานกับใบงานชิ้นใดอยู่
  - *การเปลี่ยนแปลงด้าน UI หน้าสแกนรายคน*: ลบปุ่มตั้งค่าและดรอปดาวน์ออกทั้งหมด และแทนที่ด้วยแบนเนอร์สรุปสั้นๆ ว่าขณะนี้กำลังรันสแกนด่วนกับใบงานชิ้นใดอยู่เพื่อความสะอาดตา

- **ปรับเปลี่ยนวิธีการคำนวณเปอร์เซ็นต์ความคืบหน้าของห้องเรียนบนหน้าแรก (Lobby Progress Formula Update)**:
  - แก้ไขการคำนวณเปอร์เซ็นต์ความคืบหน้า (Progress %) ของแต่ละห้องเรียนบนหน้าแดชบอร์ดหลัก (Lobby) และตารางจัดอันดับ (Leaderboard) ให้สะท้อนความสำเร็จของงานเฉลี่ยจริง แทนการนับจำนวนนักเรียนที่เริ่มส่งงาน 
  - *สูตรที่ใช้คำนวณใหม่*: นำจำนวนชิ้นงานทั้งหมดที่เด็กทุกคนส่งจริง (คะแนนสูงกว่า 0 หรือเป็น -1) หารด้วยจำนวนชิ้นงานรวมทั้งหมดที่เด็กทั้งห้องควรส่ง (จำนวนเด็กทั้งหมดในห้อง คูณด้วย จำนวนใบงานทั้งหมดของห้องเรียนนั้นๆ)
  - *ผลลัพธ์*: สะท้อนภาพรวมปริมาณงานที่นักเรียนเคลียร์ส่งเสร็จสิ้นจริงได้อย่างถูกต้องและแม่นยำ 100% ไม่แสดงผลเป็น 100% ตั้งแต่เริ่มส่งงานชิ้นแรกเหมือนกับเวอร์ชันเดิม

- **ยุบปุ่มฟังก์ชันเป็นเมนู Dropdown ย่อยบนมือถือในหน้าจัดการใบงาน (Responsive Task Actions Dropdown Menu)**:
  - แก้ไขปัญหาหน้าจอเบียดและปุ่มล้นจอในหน้าต่างจัดการใบงานห้องเรียนเมื่อแสดงผลบนอุปกรณ์มือถือ
  - *การเปลี่ยนแปลงทางด้าน UI/UX*:
    - **บนมือถือ (Mobile View)**: ยุบปุ่มเครื่องมือแก้ไขรูป/ดูรูป/ล้างคะแนน/ลบงาน ทั้งหมดรวมกันไว้ภายใต้ปุ่มเมนูหลักจุดไข่ปลาแนวตั้ง **`⋮`** เพียงปุ่มเดียวเพื่อประหยัดพื้นที่ 100%
    - **ป๊อปอัปเมนูย่อย (Submenu Dropdown)**: เมื่อแตะปุ่ม `⋮` จะมีเมนูคำสั่งลอยแสดงขึ้นมา โดยเปลี่ยนจากไอคอนดิบๆ ให้เป็นรายการปุ่มคำสั่งที่มีข้อความอธิบายชัดเจน เช่น "ตั้งค่ารูปใบงาน", "ดูรูปภาพใบงาน", "ล้างคะแนนทั้งหมด", และ "ลบใบงานนี้" (สีแดงสำหรับเตือนความปลอดภัย)
    - **บนคอมพิวเตอร์ (Desktop View)**: ยังคงแสดงปุ่มไอคอนลัดแยก 4 ปุ่มเรียงแถวตามปกติ เพื่อความสะดวกรวดเร็วในการทำงานแบบคลิกเดียวจบของคุณครู

- **ปรับปุ่มคัดลอกงานและปุ่มเพิ่มงานให้ยืดหยุ่นบนมือถือ (Responsive Header Action Buttons)**:
  - แก้ไขปัญหาปุ่ม "คัดลอกจากห้องนี้" และปุ่ม "เพิ่มงาน" ล้นขอบเขตการ์ดในหน้าต่างจัดการงานของห้องบนอุปกรณ์มือถือ
  - *การเปลี่ยนแปลงทางด้าน UI/UX*:
    - **บนมือถือ (Mobile View)**: ยุบข้อความอธิบายปุ่ม "คัดลอกจากห้องนี้" และ "เพิ่มงาน" ออกชั่วคราวบนหน้าจอขนาดเล็ก โดยแสดงผลเพียงไอคอนรูปกระดาษซ้อน (Copy) `i className="fa-solid fa-copy"` และไอคอนบวก `+` เพื่อไม่ให้ปุ่มกว้างเกินไปและไม่ล้นการ์ด
    - **บนคอมพิวเตอร์ (Desktop View)**: แสดงข้อความเต็มปกติ ("คัดลอกจากห้องนี้" และ "เพิ่มงาน") เพื่อความเข้าใจและสวยงามเมื่อมีพื้นที่กว้างเพียงพอ

- **ปรับลดระยะเยื้อง ขนาดช่องป้อน และระยะห่างของฟอร์มเพิ่มงานบนมือถือ (Optimize Spacing for Mobile Add Task Form)**:
  - ป้องกันปัญหาปุ่มเพิ่มงานเบียดหรือเกือบล้นขอบการ์ดบนมือถือบางรุ่นที่มีหน้าจอกว้างค่อนข้างแคบ (เช่น 320px - 360px)
  - *การปรับปรุงด้าน CSS/Layout*:
    - ลดระยะห่าง (`gap`) ของฟอร์มสร้างงานใหม่จาก `gap-2` เป็น `gap-1.5` บนมือถือ
    - ปรับระยะเยื้องภายใน (`padding`) ของช่องกรอกชื่อใบงานและปุ่มเพิ่มงานจากเดิม `px-4 py-2.5` ให้เหลือเพียง `px-2.5 py-2` เพื่อประหยัดพื้นที่แนวราบและแนวตั้ง
    - ปรับขนาดความกว้างของช่องกรอกคะแนนเต็มจาก `w-16` เป็น `w-12` บนจอมือถือ (เนื่องจากคะแนนส่วนมากใช้ไม่เกิน 3 หลัก ซึ่งขนาด 48px มีพื้นที่เพียงพออย่างเหลือเฟือ) และคงขนาด `w-20` ไว้สำหรับหน้าจอคอมพิวเตอร์ตามเดิม

- **Commit**: `dc4f043` (เพิ่มตัวซิงค์โคลอน), `0dbd464` (อัปเกรด dependencies tsx), `ea78774` (ย้ายสิทธิ์ config), `0b56b85` (ลบ config เก่า), `e78bde9` (สร้าง config คืนชีพ Prisma 7), `cce44e6` (สคริปต์คู่ขนาน), `b1f67fa` (Inline db URL), `5d222d9` (Placeholder fallback), `f1e5bb9` (นำเข้าไฟล์รูปใบงาน), `5e84800` (คืนค่า redirect คิวรี่จริง), `31c1f8b` (สลับหน้าฝั่ง Client), `6efe324` (เพิ่ม Image Fallback ในหน้าตรวจงานด่วน), `ba1b175` (Checkbox คะแนนด่วน), `8b4b3f1` (ระบบสแกนรับสมุดด่วน & แก้ลำดับประกาศ useEffect), `6cae4b8` (เมนูนำทางบนมือถือ Hamburger Drawer), `a2bee26` (แก้ไขความโปร่งใสและระยะเยื้องของเมนูมือถือ), `e4ae336` (แก้ไข MySQL Driver คืนค่า String บนเซิร์ฟเวอร์จริง), `09c4e92` (ปรับปรุงการนับสถานะสมุดส่งและคะแนนในหน้ารายละเอียดห้องเรียน), `4b79ef7` (ระบบให้คะแนนแบบกลุ่มและปุ่มเลือกนักเรียนทั้งหมด), `fb92ef2` (แก้ไขบั๊กล้าง localStorage แล้วเด้งหน้าจอเขียวทันที), `d32c510` (แก้ไขแสดงผลคะแนนเต็มติดลบ -1 ในหน้าตรวจคะแนน), `b064302` (ระบบกำหนดคะแนนเต็มของแต่ละใบงานและช่องกรอกจัดการคะแนนเต็ม), `383de17` (ปรับค่าคะแนนเต็มเริ่มต้นตอนเพิ่มงานจาก 10 เป็น 0 คะแนนตามความต้องการ), `371a0e0` (ป้องกันการเด้งหน้าจอเขียวเมื่อกดเปิดสวิตช์โหมดตรวจด่วนครั้งแรก), `3ff057d` (ย้ายการตั้งค่าโหมดตรวจสมุดด่วนไปไว้ที่หน้าห้องเรียนหลักพร้อมติดตั้งไฟกะพริบแจ้งสถานะ), `94dec9a` (ปรับเปลี่ยนวิธีการคำนวณ Progress % ของห้องเรียนบนหน้า Lobby ตามสัดส่วนงานจริง), `b9b3285` (ระบบจัดการปุ่มควบคุมใบงานบนมือถือด้วยปุ่ม Dropdown เมนูไข่ปลา ⋮ ย่อยแบบลดพื้นที่), `e2a0588` (ปรับปุ่มคัดลอกงานและปุ่มเพิ่มงานให้ยุบเหลือแต่ไอคอนเมื่ออยู่บนจอมือถือ) และ `fbdf2a3` (ปรับลดขนาดความกว้างช่องป้อนคะแนนเต็มและขยับระยะห่างภายในฟอร์มเพิ่มงานบนมือถือให้สมดุลยิ่งขึ้น).

## ========== CHECKPOINT (2026-07-01, จบภารกิจปรับปรุงความกระชับของฟอร์มบนมือถือรุ่นประหยัดหน้าจอ — Hand-off to Rudolf) ==========
แอปพลิเคชัน Next.js `homework-next` ปรับแต่งขนาดระยะห่างและสัดส่วนของแบบฟอร์มเพิ่มใบงานใหม่ในหน้าจอจัดการงานสำหรับโทรศัพท์รุ่นหน้าจอแคบพิเศษ ให้แสดงผลอยู่ในกรอบการ์ดอย่างหลวมและเป็นสัดส่วน ปลอดภัยจากปัญหาขอบปุ่มตกจอ เรียบร้อยสมบูรณ์แบบ
